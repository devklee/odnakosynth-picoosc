...
void processMidiMessage(uint8_t *buf, uint bufLen)
{
    ...
    if(code == 0x09 && cn == midiChannel)
    {
        ...
        // buf[1] <= 23
        //TODO procces C-2 -- C-1, control tasten?
        queueCore1Item item = {0};
        item.op = 2;    
        item.value = buf[1] | (buf[2] << 8);
        core1AddItem(&item);            
    }
    else if(code == 0x08 && cn == midiChannel)
    {
        //TODO note off
        midiNoteNoteOff(buf[1]);
    }
    else if(code == 0x0b && cn == midiChannel)
    {
        ...
        else if(buf[1] == 80 || buf[1] == 81 || buf[1] == 82 || buf[1] == 83 || buf[1] == 85 || buf[1] == 86)
        {
            queueCore1Item item = {0};
            item.op = 5;    // freq adjusting
            item.flags = 0;
            ...
            if(buf[1] == 80)
            {
                item.flags = 0x02;      // chan A, st
            }
            else if(buf[1] == 81)      // chan A, ct
            {
                item.flags = 0x00;
            }
            else if(buf[1] == 82)
            {                
                item.flags = 0x03;      // chanB, st
            }
            else if(buf[1] == 83)
            {                
                item.flags = 0x01;      // chanB, ct
            }
            else if(buf[1] == 85)
            {                
                item.flags = 0x42;      // subbass, st
            }
            else if(buf[1] == 86)
            {                
                item.flags = 0x40;      // subbass, ct
            }
